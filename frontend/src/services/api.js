...
class ApiService {
  ...
  async createPaymentIntent(propertyId, amount) {
    try {
      const res = await api.post("/api/v1/payment/create-intent", {
        propertyId,
        amount,
      });
      ...
      return {
        success: false,
        error: err.response?.data?.message || "Create intent failed",
      };
    }
  }
  ...
  async confirmPayment(paymentId, method) {
    try {
      const res = await api.post("/api/v1/payment/confirm", {
        paymentId,
      ...
      return {
        success: false,
        error: err.response?.data?.message || "Confirm payment failed",
      };
    }
  }
  ...
  async getApprovedProperties(params = {}) {
    try {
      ...
      return {
        success: false,
        error:
          error.response?.data?.message || "Failed to get approved properties",
      };
    }
  }
